...
指数定义、可视化图表与真实数据见 `references/effectiveness-metrics.md` 与仓库根 `efficiency-dashboard.html`，每一轮末填 3 个数（级别 / 轮数 / 指数）。
...
| v3.2 | 通用化（面向所有人/不绑技术栈）+ 效率指数可视化（产出级别/时间/效率指数 + dashboard.html） |
| v3.1 | 宗旨显性化 + 主循环状态机图 + Chair 执行清单 + 公开发布到 GitHub |
