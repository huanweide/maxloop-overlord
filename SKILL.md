...
version: 3.2
description: 任何 agent 都可调用的通用 skill（不绑定任何项目/语言/框架，适用任何技术栈甚至非代码任务）。当用户要求对「正式版产品」进行深度体验、集体评审与迭代修改直至所有问题归零时使用。提供多智能体「魔王系统」框架——**唯一区别在于它自带刹车**：Oracle Gate（外部判据先决）、判别测试（批评须先被证明在原代码上失败）、Repeat-Failure Breaker（同问题 2 次未解强制讨论换方案、3 次 pivot、4 次降级交放弃报告）、Patch-Depth Gate（必答根因）。**高效提升模型性能只是价值之一**——更核心的是反假收敛、防资源浪费、结论可复核，并用 efficiency-metrics 把「提升作用」量化成效率指数（产出级别/产出时间/效率指数）。
agent_created: true
---

# MaxLoop 魔王系统 v3.2 · 唯一带刹车的多智能体迭代引擎

> **宗旨（一句话）**：魔王系统是一台「自带刹车」的多智能体迭代引擎，**面向所有 agent 的通用 skill（不绑定任何项目 / 语言 / 框架，任何 agent 加载即用）**，专治一个病——**AI 自我迭代会假收敛**（一轮接一轮「全绿」，问题却还在）。它全部的设计只围绕一句话：**「修好了」的判据必须来自模型之外，永远不许模型自己说了算。** 外部命令的真实输出、修复前的失败证据、真实入口的可达性，才是放行标准。根因协议 → 判别测试 → Oracle Gate → 重复失败打破器，四件套串成一条铁链，把"高效执行"和"不假绿、不空转、不无脑重试"同时焊死。

## 零、为什么是 v3.0（先读这段，否则会重蹈覆辙）

v2.2 在**某真实项目**上烧了 **6 轮**（示意：一个识别类 bug，版本 0.6.x→0.6.y）。每一轮：

- 实现员交差 → 测试全绿（示意：358/0 → 373/0 → 397/0 → 413/0 → 421/0，轮轮全绿）
- 对抗验证员每轮都挖出新 P1 → 修 → 下轮又挖出新的 → **同一个根因在每一层都换个马甲回来**（这是假收敛的典型信号）
...
## 一、定位与触发（任何 agent 都能用）

**这是面向所有 agent 的通用 skill**：不绑定任何项目 / 语言 / 框架，任何 agent 加载即用。它治的是「AI 自我迭代会假收敛（一轮接一轮全绿，问题却还在）」这个通用病，与具体产品无关。

适用场景：当你（或任意 agent）要对已处于「正式版」状态的产品 / 项目做深度体验、集体评审与迭代修改，直至所有问题归零时使用。触发条件：用户明确要求 / 提供正式版产品 / 希望固化为循环。
...
>
> **实证**：在某真实项目用这套流程修一个同类 bug（LLM 返回 HTML 导致 JSON.parse 炸），
...
- `references/novel-forge-adapter.md` — 适配层**示例**（以某 Next.js 产品为例；作用于其他产品时由用户提供等价四项命令替换）
...
| 实证基底（v2.2 vs v3.0 示意对比） | 新增 | 用某真实项目的示意数据证明价值，非编造 |
